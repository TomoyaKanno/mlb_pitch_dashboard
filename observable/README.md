...
The Python data loaders call `pipeline.export`, which reloads and validates the durable snapshot before producing browser-ready season team totals, top-30 player totals, and per-team top-five pitcher workload lists (`dashboard.json.py`), plus the team timeseries (`team-timeseries.json.py`: daily increments plus game-grain complete games). The dashboard payload also drives Recent strain: the latest completed game with an MLB box-score link, the next scheduled game with optional probable-starter context and an MLB Gameday link, snapshot-time Eastern rest-day context, a roster-aware bullpen heatmap with row-aligned 3/5/14-day pitcher totals, and active depth-chart starter rest entering the refresh's Eastern schedule date. Starter rest uses official `gamesStarted` appearances, follows pitcher ids across trades, excludes inactive depth-chart rows, and stays null when the published season has no prior start. The built site makes no MLB API calls for pitch data; the browser may load team logos and pitcher portraits from MLB's public CDN.
