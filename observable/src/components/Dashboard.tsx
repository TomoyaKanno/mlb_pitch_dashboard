import {useEffect, useMemo, useRef, useState, type CSSProperties} from "npm:react";
import {
...
function heatCellStyle(pitches: number): CSSProperties | undefined {
  ...
  const accentShare = 20 + Math.round((Math.min(pitches, 30) / 30) * 70);
...
function RecentStrain({
  ...
  if (!selected) return <p className="secondary recent-empty">No completed team games are available in this snapshot.</p>;
  ...
  return (
    <section className="recent-strain" aria-label="Recent strain">
      ...
      <div className="recent-game-card">
        <img className="recent-team-logo" src={"https://www.mlbstatic.com/team-logos/" + selected.team_id + ".svg"} alt="" width={54} height={54} onError={(event) => { event.currentTarget.style.visibility = "hidden"; }} />
      ...
      <div className="table-shell recent-table">
        <table>
          ...
          <tbody>{selected.pitchers.map((pitcher) => <tr key={pitcher.pitcher_id}><td>{pitcher.pitcher_name}</td><td className="recent-role">{pitcher.official_started ? "SP" : "RP"}</td><td className="recent-pitches">{integer.format(pitcher.pitches)}</td></tr>)}</tbody>
        </table>
      </div>
      <section className="bullpen-usage" aria-labelledby="bullpen-usage-title">
        <div className="bullpen-usage-header">
          <div><p className="eyebrow">Recent relief usage</p><h2 id="bullpen-usage-title">Bullpen, last 14 days</h2><p className="secondary">Each cell is a pitch count; blank cells are days without a relief appearance.</p></div>
          {usage && <span className="usage-window">{formatDate(usage.dates[0])} – {formatDate(usage.end_date)}</span>}
        </div>
        {usage && usage.pitchers.length > 0 ? <div className="heatmap-scroll"><table className="bullpen-heatmap"><thead><tr><th scope="col">Pitcher</th>{usage.dates.map((day) => <th key={day} className="heat-date" scope="col" title={formatDate(day)}>{new Intl.DateTimeFormat("en-US", {month: "short", day: "numeric", timeZone: "UTC"}).format(new Date(day + "T00:00:00Z"))}</th>)}</tr></thead><tbody>{usage.pitchers.map((pitcher) => <tr key={pitcher.pitcher_id}><th scope="row">{pitcher.pitcher_name}</th>{usage.dates.map((day, index) => { const pitches = pitcher.pitches[index] ?? 0; return <td key={day} className={"heat-cell" + (pitches ? " used" : "")} style={heatCellStyle(pitches)} title={pitcher.pitcher_name + ": " + (pitches || "no") + " pitches on " + formatDate(day)} aria-label={pitcher.pitcher_name + ": " + (pitches || "no") + " pitches on " + formatDate(day)}>{pitches || ""}</td>; })}</tr>)}</tbody></table></div> : <p className="secondary">No relief appearances are available in this window.</p>}
      </section>
    </section>
  );
}
