...
The build reloads the data branch, verifies the manifest and hashes, exports browser-ready aggregates (season team totals, latest-game workloads, upcoming-game records with optional probable starters, 14-day bullpen windows, and the sibling team timeseries with daily points plus `complete_games`), requires exactly 30 teams, and checks that recent-game, populated next-game, and bullpen team IDs match the season teams; that optional probable-starter fields are paired; that bullpen pitch arrays match their 14 dates; and that the timeseries season/`data_commit` match the dashboard payload and reconcile to season totals. Legacy snapshots without `next-games.json` are accepted with an empty next-game list until the next refresh rewrites them. A failed refresh does not start a production build. A failed build does not reach the deployment job, so GitHub Pages keeps serving the last successful artifact.
