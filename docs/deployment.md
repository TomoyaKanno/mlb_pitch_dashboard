...
`Refresh dashboard data` runs every day at 09:17 UTC from March through November. During the regular season, which is entirely in EDT, that is 5:17 a.m. Eastern. Revisit the UTC conversion before a future season. The off-hour minute avoids the busiest part of GitHub's scheduler, and the time allows late West Coast games to reach final status. It can also be run manually with an optional season, force-refresh flag, and reconciliation window.

Expect the scheduled run to be dispatched well after the requested time. Measured delays over 21–25 July 2026 were 2h03m, 2h04m, 2h05m, 1h56m, and 1h28m. The cause is upstream of this repository: run creation itself is late, since `created_at` equals `run_started_at` on every run and the job is therefore not sitting queued for a runner, and `Probe GitHub schedule latency` is delayed comparably despite holding no permissions, checking out nothing, and running for eight seconds. An earlier revision of these docs attributed the delay to GitHub's timezone-aware `timezone:` key and presented the move to a plain UTC cron as the remedy. That is not supported: `"17 5"` with `America/New_York` and `"17 9"` UTC denote the same instant during EDT, so the switch changed nothing, and the delay persisted after it. Whether the remaining variation tracks scheduler load by day of week is still open.
