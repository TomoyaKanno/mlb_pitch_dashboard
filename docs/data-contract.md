...
- Role adjustment is conservative and auditable. A classified relief-dominant opener pairs with its follower, who adjusts to SP as the planned bulk man; other ambiguous long relief remains RP with `needs_review`. `config/role_overrides.json` is keyed by season: each season entry holds an `overrides` map of reviewed per-appearance exceptions (`game_pk:pitcher_id` → `{role, reason}`) plus a `reviewed_through` date marking the last manual flag review. The file is the audited record of review decisions, so it is validated strictly — a malformed entry fails the load, and an override key that matches no persisted appearance in the refreshed season fails the refresh, never a silent no-op. Refresh records the marker in the snapshot manifest, and export reads it from there as `role_reviewed_through`, so the displayed marker always describes the classifications actually persisted — a config edit alone never changes it. Manifests written before the marker existed, including frozen historical seasons, simply have none. A manual override on an opener does not cascade to its follower — reviewers set both halves of an overridden game explicitly.
