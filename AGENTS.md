...
- framing, role-basis, and per-game controls respond, including **Team total**, the top-30 **Player total** table, and top-five pitcher usage in Team total/SP/RP details;
...
- the bullpen heatmap shows 14 ordered calendar days of official-reliever pitch counts plus row-aligned 3/5/14-calendar-day pitcher totals in a visually distinct trailing panel, includes unused active depth-chart bullpen arms, and mutes IL/Minors rows (while retaining their pitch history) only when those arms pitched in the window;
